...
def gen_group_ratio_templates(prices: dict) -> str:
    """Generate one SOL_subst_ratios_{good} template per good in DEMAND_GROUPS.

    Each template shows only the outgoing ratios for that specific good:
    "1 unit of {good} → X units of {other_good}" for each other good in its group.
    Scope-agnostic (static text + icons only, no ScriptValue calls).
    """
    lines: List[str] = [
        "# ============================================================",
