...
| scope | pop | Emitting SOL goods demand values without dividing by development, causing demand to scale with location development level | Add 'divide = location.development' to every single-group demand block in SOL_goods_demand_values.txt. The EU5 engine multiplies ALL pop demands by (development/10); pre-dividing cancels it to a constant factor of 1/10. The displayed demand scale in z_SOL_group_demand_scales*.txt must NOT be divided — it is UI-only. | Intentional hidden shim: net in-game demand = simulator_scale × (1/development) × (development/10) = scale/10 (development-neutral). Multi-group aggregates inherit the division from their sub-values; do not add a second divide there. Display values in z_SOL_group_demand_scales_location.txt are the true economic scale and must remain undivided. |
