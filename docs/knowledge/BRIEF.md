...
| scope | pop | Emitting SOL goods demand values without the development cancellation shim (EU5 1.2+), or using the old EU5 pre-1.2 shim (divide=development, multiply=10) which cancels the old multiplier (development/10) but not the new one (1+development/20) | EU5 1.2+ engine multiplies all pop demands by (1+development/20). Cancel it with 'multiply = 20' AND 'divide = SOL_dev_demand_divisor' in every single-group demand block in SOL_goods_demand_values.txt. SOL_dev_demand_divisor = { value = 20  add = location.development } (minimum value 20, no div/0 risk). Net math: scale × 20/(20+dev) × (20+dev)/20 = scale. | Intentional hidden shim, updated for EU5 1.2. Pre-1.2 shim was (×10 ÷ development) cancelling the old (development/10) engine multiplier. 1.2 changed the engine multiplier to (1+development/20), so the new shim is ×20/(20+development). Multi-group aggregates inherit the cancellation from sub-values; no additional ops needed there. Display values in z_SOL_group_demand_scales_location.txt must remain undivided. |
...
| Scripted Effects | 492 | `data/index/scripted_effects.txt` | Verify name before using in `effect = { ... }` |
