...
| encoding | any | Exporting alpha (budget-share) values directly from simulator without adjustment, causing in-game spending to fall below simulated income | EXPORT_ALPHA_MULTIPLIER = 1.0 in engel_export.py (Σα = 1 preserved). The compensating ×2 now lives in-game as global_var:sol_era_coeff (initialised to 2.0, decays ×0.95/era). Both multipliers are injected in z_SOL_group_demand_scales_location.txt via 'multiply = global_var:sol_era_coeff' and 'multiply = local_sol_scarcity_adj'. To recalibrate the base: edit sol_init_export_adj in A_SOL_economy_effects.txt. | Intentional backend shim: substitute groups cause pops to choose cheapest good (not average-priced), and conditional-unlock goods are absent in early game. Together these halve actual spending vs. simulator prediction. The ×2 multiplier restores balance and is now dynamic: era decay (×0.95/era) + per-market scarcity correction (±20% via local_sol_scarcity_adj script_value). |
