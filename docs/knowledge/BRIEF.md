...
| scope | global | Initialising a global_variable referenced by script_values exclusively from a post-lobby on_action (e.g. gls_init_after_lobby), with no on_game_start fallback | Also register the same init effect under on_game_start so the global is set before any script_value referencing 'global_var:NAME' is first evaluated; lobby-time init alone leaves a brief window during new-game start where pop/market script_value evaluation emits 'Failed to fetch variable for NAME due to not being set' and 'Got value of type none' warnings, propagating to caller script_values | EU5 evaluates pop demand script_values during the early game-start window before custom post-lobby on_actions fire. global_var lookups inside multiply/add ops produce loud error.log spam if unset. Fix: hook the same init effect to on_game_start as a sibling on_action handler — idempotent, safe to run twice. |
