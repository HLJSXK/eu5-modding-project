...
> Last updated: 2026-08-07 (four-class demand purity solve)
...
- **Monthly local demand modifier** - `monthly_country_pulse` calls `sol_update_local_pop_demand_modifiers`, which computes each owned land location's income-closed raw coefficient, groups non-negligible locations by their strongest absolute four-stratum base-spending share, validates the aggregate class directions, solves the country-level class system for final coefficients, and applies `sol_local_pop_demand_modifier` for one month with `size = var:sol_location_pop_demand_modifier_size`. Class counts may be uneven; missing classes or any solve validation failure keep raw coefficients. The first post-lobby refresh primes the estate-building cache once; yearly pulse keeps it current afterward.
