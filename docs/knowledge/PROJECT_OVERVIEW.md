...
| `gen_sol_economy_effects.py` | After changing SOL economy effect emitters or demand data; called by `gen_sol_chain.py` for `stable`/`sol_standalone`/`all` | Emits the full stable and standalone `A_SOL_economy_effects.txt` files from Python builders, expanding the market refresh block from demand data |
...
| `scripts/sol_economy_effects_source.py` | Python emitters for the shared SOL economy effect blocks; target selection controls stable-only gold transfer and the standalone panel-stat variant |
...
- **Generated economy effects** - `gen_sol_economy_effects.py` makes both base-target `A_SOL_economy_effects.txt` files generated outputs. Python emitters define the Jomini effect sections, target selection handles stable-only gold-transfer scaling and the standalone panel-stat variant, and demand data expands the market refresh block.
...
- **Monthly local demand modifier** - `monthly_country_pulse` calls `sol_update_local_pop_demand_modifiers`, which computes each owned land location's income-closed raw coefficient, scores its four-stratum shares relative to nationwide shares, and classifies it with a confidence-ordered greedy pass. Classification capacity is measured as `raw * base`: every class receives 5% of nationwide raw spending as a seed floor, and the remaining 80% is divided among strata with `raw baseline - target > 0` in proportion to that downward pressure. Candidate affinity is reduced by `1.5 * current_class_raw_capacity / dynamic_class_target`, so small pure classes can use arbitrarily large positive factors while most capacity flows to classes that need bounded reduction. The resulting matrix sums each class's raw-weighted stratum spending, is row-normalized by nationwide raw stratum spending, and is solved for nonnegative factors. A successful factor multiplies every member location's raw coefficient before `sol_local_pop_demand_modifier` is applied for one month with `size = var:sol_location_pop_demand_modifier_size`; factor 1 preserves raw, 0 to 1 reduces it without crossing zero, and factors above 1 are uncapped. Missing classes or any pivot, negative-factor, or original-equation residual validation failure keep raw coefficients. The first post-lobby refresh primes the estate-building cache once; yearly pulse keeps it current afterward.
