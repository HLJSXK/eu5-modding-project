...
> Last updated: 2026-08-07 (balanced relative-structure four-class demand solve)
...
- **Monthly local demand modifier** - `monthly_country_pulse` calls `sol_update_local_pop_demand_modifiers`, which computes each owned land location's income-closed raw coefficient, scores its four-stratum shares relative to nationwide shares, and classifies it with a confidence-ordered greedy pass. Candidate affinity is reduced by `1.5 * current_class_base / (country_base / 4)`, steering nobles, clergy, burghers, and lower classes toward equal aggregate base without requiring absolute diagonal dominance. The resulting class matrix is row-normalized by nationwide stratum base for stable Gaussian elimination, then solved for final class coefficients and applied through `sol_local_pop_demand_modifier` for one month with `size = var:sol_location_pop_demand_modifier_size`. Classification balance and solve feasibility are separate: a normalized RHS above the coefficient cap makes the bounded solve impossible, while a target row ratio outside every matrix-column ratio proves that no nonnegative exact solution exists. Missing classes or any pivot, coefficient, or original-equation residual validation failure keep raw coefficients. The first post-lobby refresh primes the estate-building cache once; yearly pulse keeps it current afterward.
