...
> Last updated: 2026-08-10 (CMF solver preparation dependencies and raw-only baseline)
...
1. **Standard of Living (SOL) System** *(primary feature)* - Recalibrates baseline `demand_add` for 55 goods in `z_SOL_pop_goods.txt`, disables the engine-wide development demand multiplier, fully negates all nine vanilla per-good development thresholds, and sets `NPop.POP_NEEDS_INCOME_SCALE = 1` so strata pay the nominal cost of purchased goods instead of vanilla's 33% surcharge. It applies a monthly country-closed `local_pop_demand` coefficient after raw location spending, class assignment, and matrix aggregation. The CMF Country-level Solver tab independently controls AI and human preparation/classification, exact solve, L2/Fast, and Minimax stages; exact/L2/Fast/Minimax depend on the matching preparation toggle. With preparation disabled, the side uses a raw-only baseline that skips classification, ordering, matrix construction, approximation prefiltering, final solver diagnostics, and human solver logs while preserving raw location demand and modifier writes. Defaults preserve preparation, exact, and AI Fast/player L2, with both Minimax toggles off. Approximate candidates are accepted only when the raw-error worst stratum does not worsen beyond tolerance and mean improvement ratio is positive; otherwise `raw_fallback` preserves raw factors. Locations are classified by nobles, clergy, burghers, or lower-strata relative over-representation, with lower = commoners + tribesmen; class matrices remain raw-weighted so within-class local variation is preserved. Detailed CMF logs expose classification, matrix, exact, candidate, gate, factor, residual, and final application diagnostics. The system feeds the Living Standard situation panel, map data, mapmode, and location GUI.
