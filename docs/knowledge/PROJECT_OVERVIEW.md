...
> Last updated: 2026-07-26 (self-contained Glorp UI-based location window built into full SOL)

## Project Identity

- Mod name: **Standard of Living (SOL)**
- Mod ID: `hades.sol` | Version: `1.3.11` | Target: EU5 `1.3.11`
- Requires Community Mod Framework v2.*; Glorp UI and Construction Manager are source references, not runtime dependencies
...
11. **Built-in Glorp UI Location Window** - Full SOL's `location_window.gui` is generated from Glorp UI as a self-contained feature. The generator vendors Glorp's separately extracted vanilla location types, replaces the shared zoom button with a SOL-owned type, strips Construction Manager-only widgets/scripted-GUI/mapmode calls, then restores SOL's `local_sol_total_income` display and Living Standard tooltip. Glorp UI and Construction Manager are not runtime dependencies; SOL Standalone remains vanilla-based, while the M&T target still uses its own final window.
...
|       |   |-- events/
|       |   `-- gui/
|       |       |-- location_window.gui  generated self-contained Glorp UI-based window with SOL income and tooltip
|       |       |-- SOL_glorp_location_support.gui SOL-owned zoom-button type for the built-in window
...
| `sync_location_window.py` | After updating the Glorp UI reference; called by `gen_sol_chain.py` for `stable`/`all` | Generated self-contained Glorp-based stable location window and extracted vanilla type support, with external CM calls removed and SOL income/tooltip inserted |
...
- **Built-in Glorp UI merge** - `sync_location_window.py` validates Workshop `3601047146` as the expected source snapshot, copies its location window and separately extracted vanilla location types into full SOL, renames the shared zoom type to `sol_zoom_to_button`, strips CM-only widgets, scripted-GUI callbacks, and mapmode actions, replaces displayed total income with `local_sol_total_income`, and inserts the SOL Living Standard tooltip before the migration spacer. The generated output contains no Glorp UI or Construction Manager runtime references.
