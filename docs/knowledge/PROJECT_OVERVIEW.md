...
> Last updated: 2026-08-09 (AI proportional-fast country demand approximation)
...
| `scripts/sol_country_demand_solver_source.py` | Renderer for the separate country-level exact diagnostic, player five-strategy nonnegative hard-total solver, and AI `proportional_fast` output; it keeps the fixed-dimensional KKT and reduced minimax blocks out of the location aggregation file |
...
- **Country solver runtime cost** - For `n` owned locations, classification still contributes one confidence sort `O(n log n)` plus linear scans and matrix aggregation `O(n)`. After raw-error and nonempty-column prefilters, every exact-feasible country stops at its validated 4x4 result; exact failures run one AI `proportional_fast` candidate or the player's full strategy/minimax path. Fast has no `2^k - 1` active-set multiplier and no reduced 3x3 elimination; the exact diagnostic is fixed-dimensional. The total remains `O(n log n) + O(n) + O(1)` in location count, with player minimax adding a fixed finite-vertex multiplier. Runtime logs expose strategy `6` as `proportional_fast` and strategy `7` as exact-direct.
