...
| 2026-06 | Tried to reason about a monthly location modifier icon as if it belonged in `add_location_modifier` or the static modifier definition | Missed that modifier rows and static modifier tooltips resolve icons through `main_menu/common/modifier_icons` data | Put icon mappings in `main_menu/common/modifier_icons/<file>.txt`, using `positive`/`negative` texture paths. If unsure whether the UI asks for the static modifier id or contained modifier type, map both to the same DDS. |
