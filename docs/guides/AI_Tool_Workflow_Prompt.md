...
| 2026-06 | Tried to force a situation to use a separate custom mapmode from the situation script definition | Assumed situation `on_start` or a `map_mode = <tag>` field could switch the player's mapmode | Situation script definitions expose data-map coloring but no verified custom-mapmode field. For panel auto-switching, use a GUI `situation_panel` child with a `_show` state, gate the caller with `LateralView.IsShown` so visibility flips on each reopen, and call `[GetMapMode('<tag>').SetMapMode]`. |
...
| 2026-07 | Auto-selected the SOL mapmode when its situation panel opened but left that custom mapmode active after the panel closed | Implemented only the hidden caller widget's `_show` transition and omitted its matching exit transition | Add an `_hide` state to the same `LateralView.IsShown`-gated widget and call `[SetDefaultMapMode]`; this uses the vanilla HUD action to leave the custom mapmode without hard-coding another mapmode tag. |
