...
| 2026-06 | Used country `set_variable` flags only to feed the SOL situation GUI, producing `Variable X is set but is never used` warnings from `jomini_effect.cpp:1162` | Assumed GUI reads like `Player.MakeScope.GetVariable(...)` counted as script-side variable use; Jomini's unused-variable pass does not count GUI/localization-only reads | For GUI-only display caches, use keyed `global_variable_map` values and read them with `GetVariableFromGlobalVariableMap('cache', Scope.MakeScope)`, or ensure the scope variable is also read in a live script path. |
