...
| 2026-07 | Used full `TRY_REPLACE` copies for static-modifier and price changes that were expressible as small numeric deltas | Treated database injection as unsafe for every duplicate field instead of distinguishing additive modifier/resource entries from structural scalars and nested logic | For verified additive fields, use `TRY_INJECT` with only the desired-minus-vanilla delta; preserve `TRY_REPLACE` for structural scalar replacement, nested logic rewrites, and scripted effects/triggers. |
