...
| 2026-05 | Initialised `global_var:sol_era_coeff` only via post-lobby on_action (`gls_init_after_lobby`); script_values referencing it emitted "Failed to fetch variable" warnings during new-game start | Assumed post-lobby init was early enough; pop demand script_values are evaluated during the brief window before the lobby-dismiss callback fires | Also register the same init effect under `on_game_start` (e.g. `sol_economy_game_start`) so the global is set before any `multiply = global_var:NAME` line is first evaluated |
