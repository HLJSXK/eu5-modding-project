...
设一个国家拥有 n 个地点，划分为 K 个类别（记作 C_1 到 C_K）。

阶层集合共 4 个：贵族、教士、市民、下层。

已知量（每月在地点脉冲中算出）：

- **I_s(l)**：地点 l 的阶层 s 的净收入（流动资金）
- **B_s(l)**：地点 l 的阶层 s 的基础支出
- **m_raw(l)**：原始需求系数，即不分类时的收入加权平均

```
m_raw(l) = （各阶层 I_s(l) 之和）/（各阶层 B_s(l) 之和）
```

国家级目标，也就是每个阶层在全国范围内的流动资金总额：

```
t_s = 对所有地点 l 求和 I_s(l)      s 取四个阶层之一
```

系统矩阵 M 是一个 4 行 K 列的矩阵，第 (s, k) 元素是类别 k 中所有地点的阶层 s 的原始加权基础支出之和：

```
M[s][k] = 对类别 C_k 中所有地点 l 求和  m_raw(l) × B_s(l)
```

类别修正因子是一个 K 维向量：x = (x_1, ..., x_K)。
...
其中 t = (t_贵, t_教, t_市, t_下) 是四维目标向量。

两部分的含义：

- 等式 **M x = t**：每个阶层的全国总需求等于它的流动资金
- 约束 **x >= 0**：modifier 不能是负数（引擎硬限制）

这是一个带约束的线性系统。去掉非负约束，只要矩阵满秩解就一定存在。加上 x >= 0 之后，许多系统就变成不可行。

### 3.3 精确求解

该线性系统有解，当且仅当目标向量 t 落在 M 的列张成的**非负锥**内。

非负锥的定义是：把 M 的每一列看成一个方向向量，所有列向量的非负组合能到达的点集：

```
cone(M) = { λ_1 · M第1列 + λ_2 · M第2列 + ... + λ_K · M第K列  ，其中每个 λ >= 0 }
```

具体到第二节那组人口结构，三个类别的列向量就是（按贵族/教士/市民/下层排列，m_raw = 1）：

```
第1列（庄园区）  = (400, 150, 100, 350)
第2列（商业中心）= (150, 100, 450, 300)
第3列（农业省份）= (100,  50, 100, 750)
```

非负锥就是这三个向量的所有非负组合能够到达的点集。t 在其中就可行，不在就不可行。

设目标是贵族 +49%、下层 −20%，教士与市民不变，即 t = (968, 300, 650, 1120)。

只看贵族和教士两行就够。教士对贵族的比值，三列分别是：
...
而目标要求 300 / 968 = 0.310。任何非负组合的比值都被夹在 0.375 和 0.667 之间，永远到不了 0.310——因为教士的 50% 和贵族的 61.5% 同在庄园区，抬贵族必然连带抬教士。

这就是"锥外"的严格含义。注意它对目标很敏感：若改成教士 +40%（比值 0.434，落进区间），同一组人口结构下最大相对误差立刻降到 2.7%，几乎可行。可行与否取决于目标是否尊重人口结构里的同址关系，与算法精度无关。

由于非负锥是很窄的区域——三四个非负向量在 4 维空间张成的锥只占整个空间很小一块——大部分情况精确解都不可行。而 t 由实际收入决定，与人口结构是两套独立演化的量，完全可能落在锥外。

此外，因为每个类别的列都是若干地点列的非负求和，所以：

```
cone(任意划分)  包含于  cone(每地点一列)
```
...
*图 3：把四维系统投影到（贵族, 教士）平面。左图三条箭头是三个类别的列向量（Manor belt 庄园区 / Trade hub 商业中心 / Farm province 农业省份），青色楔形是它们的非负组合能到达的全部区域；raw 基线落在楔形内，而目标（红叉）的斜率 0.310 低于楔形下界 0.375，所以无论怎么调 x 都到不了，最近只能到蓝点（教士超出 31.6%）。右图把判定压缩成一个量：教士/贵族比值落在 0.375 到 0.667 之间才可达。注意最右侧那根——目标若改成教士 +40%，比值 0.434 就落回区间内了。*

实际求解中类别数固定为 K = 4（与阶层数相同），所以 M 是方阵，算法很直接：

1. 计算 x = M 的逆矩阵乘 t（定维高斯消元）
2. 检查是否所有分量都大于等于 0
3. 有负分量 → 不可用，转入近似
4. 否则采纳

复杂度是 K 的三次方，K = 4 时是常数。
...
方程解出来了，行列式不为零，数值上毫无问题。但第三个因子是 -23.38，引擎不接受负 modifier，所以这个解存在但不可用。实测中如此的情况很常见。所以求解器的设计目标不是 100% 可行率，而是在确定不可行的前提下找到最佳近似。
...
第一步是决定用什么衡量一个候选解的好坏。一个最直接的做法是直接用绝对残差，但这样各阶层的量纲完全不可比，贵族的流动资金可能是下层的几十倍，绝对误差会被大阶层完全主导。

所以要**逐行按目标缩放**。对每个阶层 s，定义相对残差：

```
r_s = | 该阶层实际得到的总需求 − 该阶层目标 |  /  | 该阶层目标 |
```

在此之上取两个标量。设候选解为 f：

```
L_1(f)   = 四个 r_s 的平均值      ← 平均相对偏差，衡量整体拟合
L_无穷(f) = 四个 r_s 中的最大值    ← 最坏阶层的相对偏差，衡量有没有阶层被牺牲
```

记 f = 全 1（即完全不修正）为 **raw 基线**（即上版本 SOL 算法，逐地点均衡），其两个度量记作 L_1(raw) 和 L_无穷(raw)。一个候选 f 只有在两个度量都不退化时才被接受：

```
L_1(f) <= L_1(raw)     并且     L_无穷(f) <= L_无穷(raw)
```

任一条不满足就直接丢弃，不参与比较。因为前者不满足意味着根本没有优化，后者则代表某个本就最糟的阶层变得更糟，玩家的体验会更差。

这里我们选择了 L_1 主导的约束公式。通过硬约束的候选之间，按以下规则比较（设当前最优为 f\*）：

- 若 **L_1(f) 比 L_1(f\*) 低出 1% 以上** → 显著更优，接受
- 否则若 **L_1(f) 在 L_1(f\*) 的正负 1% 之内，且 L_无穷(f) 更小** → 打平但最坏行更好，接受
- 否则拒绝

也就是说 L_1 上下各 1% 构成一个**容差带**：落在带内视为平局，转而看谁的最坏阶层更轻。这样以整体拟合为主，又不会在两个 L_1 几乎相同的候选里随机挑一个把某阶层丢掉。1% 显然是一个可以调整的数值，但我没有花费太多时间在上面，因为影响不大。

方程 M f = t 加上 f >= 0 定义了一个多面体。当目标不可达时，最优解落在**边界**上——某些分量恰好为 0，或某些方程恰好满足。这些边界点就是"顶点"，由"选哪些行参与消元、哪些列自由"的组合决定。

为了求解这些顶点，代码侧枚举了 209 个（精确行序，自由列）组合，每个走一次定维高斯消元得到候选 f，然后送进上面那套判定。最后选出最优的顶点作为近似解。
...
第一道是**尺寸过滤**：地点数少于 5 的国家直接走 raw-only 路径，连分类都不做。理由则是鸽巢原理——地点数不足 4 时矩阵最多只有 3 个非空列，本来就凑不满四阶层。

实测支持这个取舍：单地点国家的 L_1 中位数从 0.329 只能改善到 0.310，而三地点以上是 0.371 → 0.222；跳过它们砍掉 70% 的精确解失败国家计数，只放弃 17% 的遍历量。

第二道是**残差过滤**：如果 raw 的四行相对残差全部已在容差内，不修正就已经够好，扫掠不可能改进任何东西，直接跳过。不过这一层影响较小。

完整流程：

```
尺寸过滤 ──地点数 < 5──> raw-only（跳过分类与求解）
        │
     5 个地点及以上
        ↓
...
raw 误差过滤 ──四行全部达标──> 跳过（保持 raw）
   ...
   精确解（4×4 高斯消元）──成功──> 采纳，策略 id 7
      ...
      失败（奇异 / 有负分量）
        ↓
   209 顶点扫掠（硬不退化 + L_1 主序）──找到──> 采纳，策略 id 8
        │
     一个都不合格
        ↓
...
下面两张表分别用 L_1（平均相对偏差）和 L_无穷（最坏阶层偏差）衡量。
...
3. 近似解才是主体，样本 236 到 407 个国家。L_1 改善 43% 到 67%，把平均偏差从三成降到一成左右；L_无穷 改善约 50%，最坏阶层从偏离 69% 降到 35%。
...
AI 降频再省 84% 到 88%：单机局面里只有一个人类国家，其余全是 AI，它们的求解挪到各自周年脉冲，任一个月只有约十二分之一在解。raw 阶段和 modifier 写入不降频，每月照跑——所以省下的只是求解那部分，而那部分本来就占九成以上。

1743 的降频收益略低（84.0% 对 87.8%），因为那时地点总数最多，而 raw 阶段不受降频影响，它在总量里的占比更高。
