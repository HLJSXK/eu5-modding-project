...
*   **GUI-only display caches**: Jomini's unused-variable validation does not count GUI/localization-only reads of scope variables. If a value exists only to drive a panel, prefer a keyed `global_variable_map` and read it in GUI with `GetVariableFromGlobalVariableMap('cache', Scope.MakeScope)`, or make sure the scope variable is also read by live script logic.
